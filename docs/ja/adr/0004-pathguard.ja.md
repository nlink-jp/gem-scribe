# ADR-0004: パスの判定は nlink-jp/pathguard に任せる — 写しを持たない
...
ADR-0002 以来、`work_dir` の検証と、読み取りのブラックリスト（`workdir.Sensitive`）は
...
- `internal/mcp/workdir` は**薄いアダプタ**にする。持つのは次だけ:
  ...
  - `NewResolver(serverDirs...)` —— このサーバー自身のディレクトリ（今は設定ディレクトリ
    `~/.config/gem-scribe` だけ）を守る場所（`pathguard.ServerDir`）として渡し、`work_dir_required` の
...
- 呼び出し箇所（`Resolve`・`Validate`・`Sensitive`）は変えない。変わるのは組み立ての 1 行
  （`cmd/mcp_wiring.go` の `workDirResolver`）と、ゼロ値で組み立てていたテストだけである。
...
（資格情報のディレクトリ）になり、転記がそこへ書かれた。ADR-0002 の写しの頃からの穴で、image-forge の独立
レビューで見つかった。
...
- `workspace.NewManager(check)` は判定を必須の引数として受け取り、`EnsureUnder` は `<work_dir>/<workspace_id>`
  ...
  無い Manager はすべてのワークスペースを拒む。配線は `newToolDeps` 1 か所で、テストはそれを使う。
...
あれば `unsupported_format`、無ければ見つからないと答えていた。slack-mcp-extender と chrome-pilot-mcp の
独立レビューで見つかった型で、ここでは HOME を一時ディレクトリにしたテストで実測して確認した（15 組中 13 組で
答えが違った）。コードは voice-scribe と同一で、同じ修正を入れた（voice-scribe ADR-0013 の追補）。
...
## Amendment (2026-09-22): 床は Local のままにする — 理由の記録

`transcribe` の `audio` は Google に渡る（20 MB 以下は Vertex AI への要求に直接、超えると運用者の Cloud Storage
バケットを経由）。組織 ADR-021 §7 は「ファイルを外部サービスへ送るサーバーは入力を `work_dir` の下からだけ取る」と
し、組織 ADR-022 §3 は外へ出るファイルに Outbound 方針（Local に加え、`id_rsa` などの秘密の名前と、資格情報の
ディレクトリ名を含むパスをどこでも拒む）を定めている。それでも gem-scribe は Local のままにする。

- 送り先は運用者自身の Google Cloud プロジェクト（Vertex AI とそのバケット）で、第三者が読める場所ではない。
  Outbound を使う 2 つ（slack-mcp-extender のアップロードはチャンネルの参加者に見え、chrome-pilot-mcp の
  `upload_file` はページがどこへでも送れる）とは、受け手が違う。
- 資格情報のファイルを音声として渡しても、文字起こしは失敗し、中身はモデルに返らない。ホームの資格情報の場所
  （`~/.ssh` など）と `.env` は、今の Local で既に拒んでいる。すり抜けるのは、ホームの外にある秘密の名前のコピー
  （`/tmp/x/id_rsa` など）が運用者自身のプロジェクトへ送られる場合だけである。
- Outbound や `work_dir` への限定は、「どこにある録音でも渡せる」という使い方（ADR-0002）を狭める。得るものが上の
  1 件だけなので、運用者の方針（完璧より総合的なリスク評価）に従い、受け入れる残余として記録する。
...
- ADR-0002（work dir 契約）: 検証の閉じた一覧と、読み取りのブラックリスト —— その実装をここで置き換える
