...
どこを指してもよい（その場で読み、コピーしない。`~/.ssh` のような資格情報・エージェント制御の
位置は、どんな綴りで渡しても、ファイルがあってもなくても同じ答えで拒否する。
...
別の Unicode 正規化の名前とハードリンクはまだ通る。限界は [ADR-0004](docs/ja/adr/0004-pathguard.ja.md) に挙げる。
